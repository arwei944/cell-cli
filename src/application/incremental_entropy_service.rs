...
impl IncrementalEntropyService {
    ...
    fn get_changed_files(&self, project_path: &str) -> CellResult<Vec<String>> {
        ...
        if git_check.as_ref().map(|o| !o.status.success()).unwrap_or(true) {
            ...
            if output.status.success() {
                for line in String::from_utf8_lossy(&output.stdout).lines() {
                    if !line.is_empty() && !files.contains(&line.to_string()) {
                        if line.ends_with(".rs") || line.ends_with(".go") || line.ends_with(".ts") || line.ends_with(".js") {
                            files.push(line.to_string());
                        }
                    }
                }
    ...
    fn calculate_entropy_change(
        ...
        if count > 0 {
            let cached_avg = total_change / count as f64;
