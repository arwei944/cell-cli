...
use crate::domain::feature::{FeatureStatus, FeatureUnit, MountPoint};
    ...
    pub fn transition_status(
        ...
        if !can_transition(&old_status, &new_status) {
            return Err(CellError::Config(format!(
                "Cannot transition from {:?} to {:?}", old_status, new_status
            )));
        }
...
mod tests {
    ...
    fn test_create_feature() {
        ...
        let f = service.create_feature("test", "desc", Some("owner")).unwrap();
