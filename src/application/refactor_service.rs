use crate::domain::refactor_assistant::{
...
pub struct ProposalDetail {
    ...
    pub impact: Option<crate::domain::refactor_assistant::RefactorImpact>,
}
...
impl RefactorService {
    ...
    pub fn apply_refactor(&mut self, id: &str) -> CellResult<()> {
        ...
        result.map_err(|e| crate::domain::errors::CellError::Other(e.to_string()))
    }

    pub fn get_proposal_detail(&self, id: &str) -> CellResult<ProposalDetail> {
        let proposal = self.assistant.get_proposal(id)
            .map_err(|e| crate::domain::errors::CellError::Other(e.to_string()))?;
...
mod tests {
    use super::*;
    use crate::domain::refactor_assistant::{RefactorType, RefactorProposal};
    ...
    fn test_analyze_code_smells() {
        ...
        assert!(r.is_ok());
        let r = r.unwrap();
        assert!(r.total_smells > 0);
        assert!(r.total_proposals > 0);
        assert!(r.summary.critical_count + r.summary.major_count + r.summary.minor_count + r.summary.info_count == r.total_smells);
    }
