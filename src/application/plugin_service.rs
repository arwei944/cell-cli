...
use crate::domain::plugin_system::{
...
impl PluginService {
    ...
    pub fn load_plugin(&mut self, path: &str) -> CellResult<PluginInfo> {
        ...
        let instance = self.manager.get(&manifest.id).ok_or_else(|| {
            crate::domain::errors::CellError::NotFound(format!("Plugin {} not found after loading", manifest.id.0))
        })?;
    ...
    pub fn activate_plugin(&mut self, id: &str) -> CellResult<PluginInfo> {
        ...
        let instance = self.manager.get(&plugin_id).ok_or_else(|| {
            crate::domain::errors::CellError::NotFound(format!("Plugin {} not found", id))
        })?;
        Ok(PluginInfo::from(instance))
    }
    ...
    fn read_manifest(path: &str) -> CellResult<PluginManifest> {
        ...
        match ext {
            Some("json") => serde_json::from_str(&content).map_err(|e| crate::domain::errors::CellError::Serde(e)),
            Some("yaml") | Some("yml") => serde_yaml::from_str(&content).map_err(|e| crate::domain::errors::CellError::Yaml(e)),
            Some("toml") => toml::from_str(&content).map_err(|e| crate::domain::errors::CellError::Toml(e)),
            _ => Ok(serde_json::from_str(&content).map_err(|e| crate::domain::errors::CellError::Serde(e))?),
        }
    }
    ...
    pub fn format_plugin_status(&self, info: &PluginInfo) -> String {
        ...
        if let Some(loaded_at) = &info.loaded_at {
            output.push_str(&format!("加载时间:    {}\n", loaded_at));
        }

        if let Some(activated_at) = &info.activated_at {
            output.push_str(&format!("激活时间:    {}\n", activated_at));
        }
...
mod tests {
    use super::*;
    use crate::domain::plugin_system::{Permission, PluginKind, PluginManifest};
