...
impl DiffStatus {
    ...
    pub fn label(&self) -> &str {
        match self {
            ...
            DiffStatus::Renamed => "重命名",
            DiffStatus::Unknown => "未知",
        }
    }
}
...
impl GitIntegrationService {
    ...
    pub fn list_branches(&self, project_path: &str) -> CellResult<Vec<BranchInfo>> {
        ...
        for line in output.lines() {
            ...
            let mut ahead = 0;
            let mut behind = 0;
            if let Ok(sync) = self.run_git(project_path, &["rev-list", "--left-right", "--count", format!("{}...@{{u}}", name).as_str()]) {
    ...
    pub fn get_diff_stats(&self, project_path: &str, target: Option<&str>) -> CellResult<DiffStat> {
        ...
        let files_changed = if let Some(p) = parts.first() {
            p.trim().split_whitespace().next().and_then(|s| s.parse().ok()).unwrap_or(0)
        } else {
            0
        };
