...
impl BenchmarkService {
    ...
    pub fn run_benchmarks(&self, project_path: &str) -> CellResult<Vec<BenchmarkResult>> {
        ...
        results.push(self.benchmark_build(project_path)?);
        results.push(self.benchmark_tests(project_path)?);
        results.push(self.benchmark_entropy(project_path)?);
        results.push(self.benchmark_arch_validate(project_path)?);
    ...
    fn benchmark_entropy(&self, project_path: &str) -> CellResult<BenchmarkResult> {
        ...
        let result = crate::application::entropy_service::run_entropy_check(project_path);
        let duration = start.elapsed();
    ...
    fn load_history(&self, project_path: &str) -> CellResult<BenchmarkHistory> {
        ...
        let history: BenchmarkHistory = serde_json::from_str(&content)
            .map_err(|e| crate::domain::errors::CellError::Config(format!("Invalid benchmark history: {}", e)))?;
        Ok(history)
    }
}
