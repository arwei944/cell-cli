...
impl StepProgress {
    ...
    pub fn set_sub_steps<S: Into<String>>(&mut self, sub_steps: Vec<S>) {
        if self.current_index < self.steps.len() {
            self.steps[self.current_index].sub_steps = sub_steps.into_iter().map(|s| s.into()).collect();
        }
