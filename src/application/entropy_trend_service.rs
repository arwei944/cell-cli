...
use crate::domain::entropy::{EntropyDimensions, EntropyGrade, EntropyReport};
...
impl EntropyTrendService {
    ...
    pub fn analyze(&self, project_path: &str) -> CellResult<EntropyTrend> {
        ...
        if trend == TrendDirection::Increasing && change_rate > 5.0 {
            warnings.push(format!("⚠️  熵值正在快速上升！变化率: {:.1}%", change_rate));
        }
        if overall_score > 60.0 {
            warnings.push(format!("⚠️  当前熵值 ({:.1}) 已超过警告阈值", overall_score));
        }
    ...
    fn run_entropy_at_commit(&self, project_path: &str, commit: &str) -> CellResult<EntropyReport> {
        ...
        if stash_output.as_ref().map(|o| o.status.success()).unwrap_or(false) {
            let _ = Command::new("git")
                .args(["stash", "pop"])
                .current_dir(project_path)
                .output();
    ...
    fn get_current_commit(&self, project_path: &str) -> CellResult<String> {
        let output = Command::new("git")
            ...
            .current_dir(project_path)
            .output()
            .map_err(|e| crate::domain::errors::CellError::Config(format!("Git command failed: {}", e)))?;
