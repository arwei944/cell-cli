...
use crate::domain::pattern_library::{Pattern, PatternCategory, PatternId, PatternLibrary, PatternStatus};
...
impl PatternLibraryService {
    ...
    pub fn list_patterns(&self) -> CellResult<Vec<PatternSummary>> {
        ...
        patterns.sort_by(|a, b| b.updated_at.cmp(&a.updated_at));
    ...
    pub fn get_pattern_detail(&self, id: &str) -> CellResult<PatternDetail> {
        ...
        let pattern = self.library.get_pattern(&pattern_id)
            .ok_or_else(|| crate::domain::errors::CellError::NotFound(format!("Pattern {} not found", id)))?;
    ...
    pub fn recommend_patterns(&self) -> CellResult<Vec<PatternRecommendation>> {
        ...
        for (i, pattern) in top_rated.iter().enumerate() {
            ...
            recommendations.push(PatternRecommendation {
                ...
                reason,
                similarity_score: (5.0 - i as f64 * 0.3).max(2.0),
            });
...
mod tests {
    use super::*;
    use crate::domain::pattern_library::{PatternExample, PatternProsCons, PatternRating, PatternRelation, PatternRelationType, PatternVersion};
    use chrono::Utc;
