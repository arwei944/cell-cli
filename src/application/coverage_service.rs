...
impl CoverageService {
    ...
    pub fn analyze(&self) -> CellResult<CoverageReport> {
        ...
        for module in modules.values_mut() {
            ...
            module.coverage_ratio = (test_func_per_file.min(1.0) * 0.4 + file_coverage * 0.6) * 100.0;
        }
        ...
        let overall_coverage = if source_files.len() > 0 {
    ...
    fn find_test_file(&self, source_file: &str, test_files: &[String]) -> Option<String> {
        ...
        for tf in test_files {
            if tf.contains(&stem) || tf.ends_with(&format!("{}.rs", stem.split('/').last().unwrap_or(""))) {
