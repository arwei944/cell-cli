...
use crate::domain::extension::{ExtensionRegistry, ExtensionType};
...
impl FeatureLifecycleState {
    pub fn description(&self) -> &'static str {
        match self {
            FeatureLifecycleState::Draft => "草稿 - 功能开发中，未发布",
            FeatureLifecycleState::Active => "活跃 - 功能已发布，可挂载",
            FeatureLifecycleState::Mounted => "已挂载 - 功能正在运行",
            FeatureLifecycleState::Suspended => "已暂停 - 功能已暂停使用",
            FeatureLifecycleState::Retired => "已退役 - 功能已废弃，待删除",
        }
    }

    pub fn can_transition_to(&self, next: &FeatureLifecycleState) -> bool {
        matches!(
            (self, next),
            (FeatureLifecycleState::Draft, FeatureLifecycleState::Active)
                | (FeatureLifecycleState::Active, FeatureLifecycleState::Mounted)
                | (FeatureLifecycleState::Mounted, FeatureLifecycleState::Suspended)
                | (FeatureLifecycleState::Suspended, FeatureLifecycleState::Mounted)
                | (FeatureLifecycleState::Mounted, FeatureLifecycleState::Retired)
                | (FeatureLifecycleState::Suspended, FeatureLifecycleState::Retired)
                | (FeatureLifecycleState::Active, FeatureLifecycleState::Retired)
                | (FeatureLifecycleState::Retired, FeatureLifecycleState::Draft)
        )
    }
}
...
impl MountPhase {
    pub fn order(&self) -> u8 {
        match self {
            MountPhase::Prepare => 1,
            MountPhase::Validate => 2,
            MountPhase::Activate => 3,
            MountPhase::Commit => 4,
            MountPhase::Rollback => 0,
        }
    }
}
...
impl FeatureRuntime {
    ...
    pub fn create_feature(&mut self, name: impl Into<String>, description: impl Into<String>) -> CellResult<&FeatureUnit> {
        ...
        if self.features.contains_key(&name) {
            return Err(CellError::AlreadyExists(format!("Feature '{}' already exists", name)));
        }
    ...
    pub fn unmount(&mut self, name: &str) -> CellResult<()> {
        if !self.mounted.contains(&name.to_string()) {
            return Err(CellError::Config(format!("Feature '{}' is not mounted", name)));
        }
    ...
    fn run_phase(&self, _name: &str, _action: &MountAction, phase: &MountPhase) -> bool {
        match phase {
            MountPhase::Prepare => true,
            MountPhase::Validate => true,
            MountPhase::Activate => true,
            MountPhase::Commit => true,
            MountPhase::Rollback => true,
        }
    }
