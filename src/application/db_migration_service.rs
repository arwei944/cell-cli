...
impl ChangeType {
    pub fn label(&self) -> &str {
        match self {
            ChangeType::CreateTable => "创建表",
            ChangeType::DropTable => "删除表",
            ChangeType::AddColumn => "添加列",
            ChangeType::DropColumn => "删除列",
            ChangeType::ModifyColumn => "修改列",
            ChangeType::AddIndex => "添加索引",
            ChangeType::DropIndex => "删除索引",
            ChangeType::Custom => "自定义",
        }
    }
}
...
impl DbMigrationService {
    ...
    pub fn get_migration(&self, project_path: &str, id: &str) -> CellResult<Migration> {
        ...
        history.migrations.into_iter().find(|m| m.id == id)
            .ok_or_else(|| crate::domain::errors::CellError::Config(format!("Migration '{}' not found", id)))
    }
    ...
    pub fn migrate(&self, project_path: &str, target_version: Option<&str>) -> CellResult<Vec<Migration>> {
        ...
        let pending: Vec<&mut Migration> = history.migrations.iter_mut()
            ...
            .filter(|m| target_version.map_or(true, |t| m.version.as_str() <= t))
            .collect();
    ...
    pub fn detect_drift(&self, project_path: &str, actual_schema: &str) -> CellResult<DriftResult> {
        ...
        let expected_hash = history.schema_hash.clone();
    ...
    fn load_history(&self, project_path: &str) -> CellResult<MigrationHistory> {
        ...
        let history: MigrationHistory = serde_json::from_str(&content)
            .map_err(|e| crate::domain::errors::CellError::Config(format!("Invalid migration history: {}", e)))?;
        Ok(history)
    }
