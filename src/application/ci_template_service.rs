use crate::application::enforcement_service::CiProvider;
...
impl CiTemplateService {
    ...
    pub fn generate_all(&self, project_path: &str) -> CellResult<Vec<CiTemplate>> {
        ...
        templates.push(self.generate_github_actions(project_path)?);
        templates.push(self.generate_gitlab_ci(project_path)?);
        templates.push(self.generate_jenkinsfile(project_path)?);
        templates.push(self.generate_gitee_workflow(project_path)?);
        Ok(templates)
    }
    ...
    pub fn apply_template(&self, project_path: &str, provider: &CiProvider) -> CellResult<CiTemplate> {
        let template = match provider {
            ...
            CiProvider::None => return Err(crate::domain::errors::CellError::Config("No provider selected".to_string())),
        };
