use crate::application::arch_service::{ArchitectureRules, ValidationResult};
...
impl ArchitectureVisualizer {
    ...
    fn generate_mermaid(&self, validation: &ValidationResult, _rules: &ArchitectureRules, options: VisualizationOptions) -> String {
        ...
        for (i, layer) in layer_order.iter().enumerate() {
            ...
            let violations = stats.map(|s| s.violations).unwrap_or(0);
        ...
        if options.include_violations && !validation.violations.is_empty() {
            ...
            for (_i, v) in validation.violations.iter().enumerate() {
    ...
    fn generate_ascii(&self, validation: &ValidationResult, _rules: &ArchitectureRules, options: VisualizationOptions) -> String {
        ...
        for (i, layer) in layer_order.iter().enumerate() {
            ...
            let violations = stats.map(|s| s.violations).unwrap_or(0);
            let file_count = stats.map(|s| s.file_count).unwrap_or(0);
            let internal_deps = stats.map(|s| s.internal_deps).unwrap_or(0);
            let external_deps = stats.map(|s| s.external_deps).unwrap_or(0);
            ...
            if options.include_stats {
                output.push_str(&format!("{} files, {} internal, {} external, {} violations", 
                    file_count, internal_deps, external_deps, violations));
            }
