use crate::application::fingerprint_service::FingerprintService;
use crate::domain::errors::CellResult;
use crate::domain::fingerprint::{FingerprintCategory, FingerprintSeverity, ProblemFingerprint};
    ...
    match args.sub {
                    ...
                    if let Ok(content) = fs::read_to_string(path) {
                        ...
                        for m in matches {
                            if let Some(cat) = &category_filter {
                                if &m.fingerprint.category != cat {
                                    continue;
                                }
                            }
                            if let Some(sev) = &severity_filter {
                                if &m.fingerprint.severity != sev {
                                    continue;
                                }
                            }
        ...
        DiagnoseSub::Fix { id } => {
            ...
            match service.fix_problem(&id) {
                ...
                Err(e) => {
                    println!("❌ 修复失败: {}", e);
                    println!("   使用 `cell diagnose detail {}` 查看指纹详情", id);
                }
            }
        }
    }
