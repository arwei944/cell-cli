...
fn print_decision_list(decisions: &[crate::domain::decision::DecisionRecord], format: Option<&str>) {
    if format == Some("json") {
...
fn print_decision_metrics(metrics: &crate::domain::decision::DecisionMetrics) {
    println!("📊 决策统计:");
...
fn parse_decision_category(s: &str) -> CellResult<DecisionCategory> {
    DecisionCategory::from_str(s).ok_or_else(|| CellError::Config(format!(
        "Unknown decision category: {}. Valid: arch, tech, process, tool, design, test, deploy, security, perf, other",
        s
    )))
}
...
fn parse_decision_status(s: &str) -> CellResult<DecisionStatus> {
    match s.to_lowercase().as_str() {
        ...
        _ => Err(CellError::Config(format!(
            "Unknown decision status: {}. Valid: proposed, accepted, rejected, deprecated, superseded",
            s
        ))),
    }
}
