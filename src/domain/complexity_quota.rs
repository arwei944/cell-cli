...
impl QuotaLevel {
    pub fn label(&self) -> &str {
        match self {
            QuotaLevel::System => "System",
            QuotaLevel::Team => "Team",
            QuotaLevel::Cell => "Cell",
            QuotaLevel::Feature => "Feature",
        }
    }
...
impl QuotaManager {
    ...
    pub fn add_quota(&mut self, quota: ComplexityQuota) {
        ...
        self.hierarchy.entry(level).or_insert_with(Vec::new).push(id);
    }
        ...
        for quota in self.quotas.values() {
            if let Some(parent_id) = &quota.parent_id {
                if !self.quotas.contains_key(parent_id) {
                    ...
                    if parent.level.order() >= quota.level.order() {
                        errors.push(format!(
