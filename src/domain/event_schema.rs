...
pub enum EventFieldType {
    ...
    Timestamp,
    Nested(String),
    Array(Box<EventFieldType>),
    Optional(Box<EventFieldType>),
}

impl EventFieldType {
    pub fn description(&self) -> String {
        match self {
            EventFieldType::String => "string".to_string(),
            EventFieldType::Int32 => "int32".to_string(),
            EventFieldType::Int64 => "int64".to_string(),
            EventFieldType::Float32 => "float".to_string(),
            EventFieldType::Float64 => "double".to_string(),
            EventFieldType::Bool => "bool".to_string(),
            EventFieldType::Bytes => "bytes".to_string(),
            EventFieldType::Timestamp => "timestamp".to_string(),
            EventFieldType::Nested(name) => format!("message({})", name),
            EventFieldType::Array(inner) => format!("repeated {}", inner.description()),
            EventFieldType::Optional(inner) => format!("optional {}", inner.description()),
        }
    }

    pub fn is_compatible_with(&self, other: &EventFieldType) -> bool {
        match (self, other) {
            (EventFieldType::String, EventFieldType::String) => true,
            (EventFieldType::Int32, EventFieldType::Int32) => true,
            (EventFieldType::Int64, EventFieldType::Int64) => true,
            (EventFieldType::Int32, EventFieldType::Int64) => true,
            (EventFieldType::Float32, EventFieldType::Float32) => true,
            (EventFieldType::Float64, EventFieldType::Float64) => true,
            (EventFieldType::Float32, EventFieldType::Float64) => true,
            (EventFieldType::Bool, EventFieldType::Bool) => true,
            (EventFieldType::Bytes, EventFieldType::Bytes) => true,
            (EventFieldType::Timestamp, EventFieldType::Timestamp) => true,
            (EventFieldType::Nested(a), EventFieldType::Nested(b)) => a == b,
            (EventFieldType::Array(a), EventFieldType::Array(b)) => a.is_compatible_with(b),
            (EventFieldType::Optional(a), EventFieldType::Optional(b)) => a.is_compatible_with(b),
            (_, EventFieldType::Optional(b)) => self.is_compatible_with(b),
            _ => false,
        }
    }
}
...
impl EventSchema {
    ...
    pub fn check_compatibility(&self, old: &EventSchema) -> CompatibilityResult {
...
impl EventSchemaRegistry {
    ...
    pub fn register(&mut self, schema: EventSchema) -> Result<&EventSchema, String> {
        ...
        let versions = self.schemas.entry(name.clone()).or_insert_with(Vec::new);
