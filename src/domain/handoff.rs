...
impl HandoffPackage {
    ...
    pub fn to_markdown(&self) -> String {
            ...
            if !progress.blockers.is_empty() {
                ...
                for blocker in &progress.blockers {
                    let status = match blocker.status {
                        crate::domain::progress::BlockerStatus::Active => "🔴 活跃",
                        crate::domain::progress::BlockerStatus::Resolved => "🟢 已解决",
                        crate::domain::progress::BlockerStatus::Bypassed => "🟡 已绕过",
                    };
        ...
        md.push_str(&format!("- **构建状态**: {}\n", build_str));
        md.push_str(&format!("- **测试状态**: {}\n", test_str));
