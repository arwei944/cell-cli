...
impl Permission {
    pub fn label(&self) -> &str {
        match self {
            Permission::ReadFileSystem => "read:fs",
            Permission::WriteFileSystem => "write:fs",
            Permission::ExecuteCommand => "execute:command",
            Permission::NetworkAccess => "network:access",
            Permission::ModifyConfig => "modify:config",
            Permission::RegisterCommand => "register:command",
            Permission::HookIntoLifecycle => "hook:lifecycle",
            Permission::AccessEntropyData => "access:entropy",
            Permission::AccessArchitectureData => "access:architecture",
            Permission::All => "*",
        }
    }
}
...
impl PluginLifecycleEvent {
    pub fn label(&self) -> &str {
        match self {
            PluginLifecycleEvent::PreInit => "pre_init",
            PluginLifecycleEvent::PostInit => "post_init",
            PluginLifecycleEvent::PreAnalyze => "pre_analyze",
            PluginLifecycleEvent::PostAnalyze => "post_analyze",
            PluginLifecycleEvent::PreGenerate => "pre_generate",
            PluginLifecycleEvent::PostGenerate => "post_generate",
            PluginLifecycleEvent::PreValidate => "pre_validate",
            PluginLifecycleEvent::PostValidate => "post_validate",
            PluginLifecycleEvent::Shutdown => "shutdown",
        }
    }
}
...
impl PluginManager {
    ...
    pub fn deactivate(&mut self, id: &PluginId) -> Result<(), PluginError> {
        ...
        let active_dependents: Vec<_> = dependents.iter()
            .filter(|d| self.plugins.get(d).map(|p| p.is_active()).unwrap_or(false))
            .collect();
    ...
    pub fn validate_dependencies(&self) -> Vec<DependencyError> {
        ...
        if let Ok(()) = self.check_circular_deps() {
        } else {
    ...
    fn check_circular_deps(&self) -> Result<(), PluginError> {
        ...
        for plugin_id in &self.load_order {
            if !visited.contains(plugin_id) {
                if self.dfs_cycle(plugin_id, &mut visited, &mut stack) {
...
impl std::fmt::Display for PluginError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            PluginError::IncompatibleHost(msg) => write!(f, "Incompatible host: {}", msg),
            PluginError::MissingDependency(dep) => write!(f, "Missing dependency: {}", dep),
            PluginError::DependencyNotLoaded(dep) => write!(f, "Dependency not loaded: {}", dep),
            PluginError::DependencyNotActive(dep) => write!(f, "Dependency not active: {}", dep),
            PluginError::HasDependents(msg) => write!(f, "{}", msg),
            PluginError::HasActiveDependents(msg) => write!(f, "{}", msg),
            PluginError::InvalidState(msg) => write!(f, "Invalid state: {}", msg),
            PluginError::CircularDependency => write!(f, "Circular dependency detected"),
...
mod tests {
    ...
    fn create_test_manifest(id: &str, name: &str) -> PluginManifest {
        PluginManifest {
            ...
            entry_point: format!("plugin_{}.wasm", id),
    ...
    fn test_plugin_with_dependency() {
        ...
        manager.register(dep.clone()).unwrap();
    ...
    fn test_unregister_plugin() {
        ...
        assert!(manager.get(&PluginId("p1".to_string())).is_none());
    }
