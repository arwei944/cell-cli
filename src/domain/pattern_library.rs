...
impl PatternCategory {
    pub fn label(&self) -> &str {
        match self {
            ...
            PatternCategory::AntiPattern => "AntiPattern",
        }
    }
}
...
impl PatternStatus {
    ...
    pub fn can_transition_to(&self, next: &PatternStatus) -> bool {
        matches!(
            (self, next),
            (PatternStatus::Draft, PatternStatus::Active)
                | (PatternStatus::Active, PatternStatus::Deprecated)
                | (PatternStatus::Deprecated, PatternStatus::Active)
        )
    }
}
...
impl Pattern {
    ...
    pub fn rate(&mut self, score: u8) {
        let score = score.clamp(1, 5) as f64;
        let total = self.rating.total_ratings as f64;
        self.rating.average_score = (self.rating.average_score * total + score) / (total + 1.0);
                ...
                if let Some(target) = self.patterns.get(&rel.target_id) {
                    ...
                    for r2 in &target.relations {
                        if let Some(snd) = self.patterns.get(&r2.target_id) {
                            if seen.insert(&snd.id) { recs.push(snd); }
                        }
                    }
                }
            }
