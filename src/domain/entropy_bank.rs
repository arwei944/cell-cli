...
impl AccountType {
    pub fn label(&self) -> &str {
        match self {
            ...
            AccountType::Cell => "Cell",
            AccountType::Feature => "Feature",
        }
    }
}
...
impl EntropyAccount {
    ...
    pub fn transfer_to(&mut self, to: &mut EntropyAccount, amount: f64) -> Result<(), BankError> {
        if amount < 0.0 {
...
impl std::fmt::Display for BankError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            BankError::InsufficientFunds(msg) => write!(f, "Insufficient funds: {}", msg),
            BankError::InvalidAmount(msg) => write!(f, "Invalid amount: {}", msg),
            BankError::DuplicateAccount(id) => write!(f, "Account already exists: {}", id),
            BankError::InvalidOperation(msg) => write!(f, "Invalid operation: {}", msg),
        }
    }
}
        ...
        match account_id {
            Some(id) => {
                self.transactions
                    .iter()
                    .filter(|t| {
                        t.from.as_ref().map(|a| a == id).unwrap_or(false)
                            || t.to.as_ref().map(|a| a == id).unwrap_or(false)
                    })
                    .collect()
            }
