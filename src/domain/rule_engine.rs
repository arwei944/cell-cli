...
impl fmt::Display for RuleType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            RuleType::Calculation => write!(f, "Calculation"),
            RuleType::Transformation => write!(f, "Transformation"),
            RuleType::Decision => write!(f, "Decision"),
            RuleType::Notification => write!(f, "Notification"),
        }
    }
}
...
impl fmt::Display for ConditionOperator {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ConditionOperator::StartsWith => write!(f, "starts_with"),
            ConditionOperator::EndsWith => write!(f, "ends_with"),
...
impl RuleCondition {
    ...
    pub fn evaluate(&self, context: &HashMap<String, serde_json::Value>) -> bool {
        let ctx_value = match context.get(&self.key) {
            Some(v) => v,
            None => return false,
        };
    ...
    fn in_list(left: &serde_json::Value, right: &serde_json::Value) -> bool {
        match right.as_array() {
            Some(arr) => arr.iter().any(|v| v == left),
            None => false,
        }
    }
}
        ...
        match &self.action_type {
            ...
            ActionType::AddValue => {
                if let Some(target) = &self.target {
                    ...
                    let old_num = old.as_ref().and_then(|v| v.as_f64()).unwrap_or(0.0);
...
impl fmt::Display for RuleEngineError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            RuleEngineError::RuleSetNotFound(id) => write!(f, "Rule set not found: {}", id),
            RuleEngineError::RuleAlreadyExists(id) => write!(f, "Rule already exists: {}", id),
            RuleEngineError::RuleSetAlreadyExists(id) => {
                write!(f, "Rule set already exists: {}", id)
            }
            RuleEngineError::VersionNotFound(id) => write!(f, "Version not found: {}", id),
            RuleEngineError::InvalidStatusTransition(msg) => {
                write!(f, "Invalid status transition: {}", msg)
            }
            RuleEngineError::CircularDependency(id) => {
                write!(f, "Circular dependency detected: {}", id)
            }
            RuleEngineError::DependencyNotFound(id) => {
                write!(f, "Dependency not found: {}", id)
            }
            RuleEngineError::EvaluationFailed(msg) => {
...
impl RuleEngine {
    ...
    pub fn update_rule(&mut self, rule_id: Uuid, updated_rule: Rule) -> RuleEngineResult<u32> {
        ...
        self.rules.insert(rule_id, updated_rule.clone());
    ...
    pub fn rollback_to_version(&mut self, rule_id: Uuid, version: u32) -> RuleEngineResult<()> {
        ...
        let target_version = versions
            ...
            .cloned()
            .ok_or(RuleEngineError::VersionNotFound(format!(
                "Version {} for rule {}",
                version, rule_id
            )))?;
        ...
        self.rules.insert(rule_id, target_version.rule.clone());
    ...
    pub fn get_rule_set(&self, rule_set_id: Uuid) -> RuleEngineResult<&RuleSet> {
        self.rule_sets
            .get(&rule_set_id)
            .ok_or(RuleEngineError::RuleSetNotFound(rule_set_id.to_string()))
    }
...
mod tests {
    ...
    fn test_evaluate_rule_not_matched() {
        ...
        let result = engine.evaluate_rule(rule_id, &mut context).unwrap();
        assert!(!result.matched);
        assert!(context.get("approved").is_none());
    }
    ...
    fn test_rule_set_operations() {
        ...
        let rules = engine.get_rules_by_set(set_id).unwrap();
