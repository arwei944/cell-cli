...
pub enum TransactionState { Init, Preparing, Prepared, Committing, Committed, Aborting, Aborted }
impl Default for TransactionState { fn default() -> Self { TransactionState::Init } }
impl fmt::Display for TransactionState {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", match self {
            TransactionState::Init => "Init", TransactionState::Preparing => "Preparing",
            TransactionState::Prepared => "Prepared", TransactionState::Committing => "Committing",
            TransactionState::Committed => "Committed", TransactionState::Aborting => "Aborting",
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub enum ParticipantState { Init, Preparing, Prepared, Committing, Committed, Aborting, Aborted }
impl Default for ParticipantState { fn default() -> Self { ParticipantState::Init } }
impl fmt::Display for ParticipantState {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", match self {
            ParticipantState::Init => "Init", ParticipantState::Preparing => "Preparing",
            ParticipantState::Prepared => "Prepared", ParticipantState::Committing => "Committing",
            ParticipantState::Committed => "Committed", ParticipantState::Aborting => "Aborting",
...
impl fmt::Display for VoteResult {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", match self { VoteResult::Yes => "Yes", VoteResult::No => "No" })
    }
}
...
impl Participant {
    ...
    pub fn commit(&mut self, tx_id: &str) -> Result<(), String> {
        ...
        let r = if let Some(f) = &self.commit_fn { f(tx_id) } else { Ok(()) };
    ...
    pub fn abort(&mut self, tx_id: &str) -> Result<(), String> {
        ...
        let r = if let Some(f) = &self.abort_fn { f(tx_id) } else { Ok(()) };
...
impl TwoPhaseTransaction {
    ...
    pub fn all_prepared(&self) -> bool {
        !self.participant_ids.is_empty() && self.participant_ids.iter()
            .all(|id| self.prepare_results.get(id).map(|r| r.is_yes()).unwrap_or(false))
    }
...
impl TwoPhaseCoordinator {
    ...
    pub fn prepare(&mut self, tx_id: &str) -> Result<&TwoPhaseTransaction, TwoPhaseCommitError> {
        ...
        if tx.any_voted_no() {
            ...
            tx.mark_aborting(format!("Participant '{}' voted No", voter));
            self.do_abort(tx_id)?;
    ...
    fn do_abort(&mut self, tx_id: &str) -> Result<(), TwoPhaseCommitError> {
        ...
        for pid in &pids {
            if let Some(p) = self.participants.get_mut(pid) {
                if p.state != ParticipantState::Aborted && p.state != ParticipantState::Init {
                    if let Err(e) = p.abort(tx_id) { errors.push((pid.clone(), e)); }
                }
            }
        }
        let tx = self.transactions.get_mut(tx_id).unwrap();
...
impl fmt::Display for TwoPhaseCommitError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            TwoPhaseCommitError::ParticipantNotFound(id) => write!(f, "Participant not found: {}", id),
            TwoPhaseCommitError::TransactionNotFound(id) => write!(f, "Transaction not found: {}", id),
            TwoPhaseCommitError::TransactionAlreadyExists(id) => write!(f, "Transaction already exists: {}", id),
            TwoPhaseCommitError::InvalidState(s, m) => write!(f, "Invalid state '{}': {}", s, m),
            TwoPhaseCommitError::CommitFailed(m) => write!(f, "Commit failed: {}", m),
            TwoPhaseCommitError::Timeout(m) => write!(f, "Timeout: {}", m),
        }
    }
}
...
mod tests {
    ...
    fn mk_fail_commit(id: &str) -> Participant {
        let i = id.to_string();
        Participant::new(id, format!("p-{}", id)).with_commit(move |_| Err(format!("{}: commit failed", i)))
    }
