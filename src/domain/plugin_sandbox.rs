...
impl Permission {
    ...
    pub fn matches(&self, required: &Permission) -> bool {
        if self == &Permission::All || required == &Permission::All {
            return true;
        }
        self == required
    }
...
impl AuditLogAction {
    pub fn label(&self) -> &str {
        match self {
            AuditLogAction::SandboxCreated => "sandbox_created",
            AuditLogAction::SandboxDestroyed => "sandbox_destroyed",
            AuditLogAction::PluginExecuted => "plugin_executed",
            AuditLogAction::PermissionGranted => "permission_granted",
            AuditLogAction::PermissionDenied => "permission_denied",
            AuditLogAction::ResourceLimitExceeded => "resource_limit_exceeded",
            AuditLogAction::PolicyUpdated => "policy_updated",
        }
    }
}
...
impl std::fmt::Display for SandboxError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            SandboxError::ExecutionTimeout(msg) => write!(f, "Execution timeout: {}", msg),
            SandboxError::MemoryLimitExceeded(msg) => write!(f, "Memory limit exceeded: {}", msg),
            SandboxError::CallLimitExceeded(msg) => write!(f, "Call limit exceeded: {}", msg),
            SandboxError::PolicyNotFound(id) => write!(f, "Policy not found: {}", id),
        }
    }
}
...
impl SandboxManager {
    ...
    pub fn update_policy(&mut self, policy_id: &str, limit: ResourceLimit) -> Result<(), SandboxError> {
        ...
        self.add_audit_log(
            ...
            format!("Policy {} updated", policy_id),
            true,
        );
    ...
    pub fn execute_plugin(
        ...
        self.add_audit_log(
            ...
            format!(
                "Executed in {}ms, using {} bytes",
                duration_ms, memory_used_bytes
            ),
            true,
        );
...
mod tests {
    ...
    fn test_policy_update() {
        ...
        let result = manager.update_policy("test-policy", new_limit.clone());
        assert!(result.is_ok());
