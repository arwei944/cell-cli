...
impl ReleaseStatus {
    pub fn label(&self) -> &str {
        match self {
            ...
            ReleaseStatus::Progressing => "Progressing",
    ...
    pub fn is_terminal(&self) -> bool {
        matches!(
            self,
            ReleaseStatus::Promoted | ReleaseStatus::RolledBack | ReleaseStatus::Failed
        )
    }

    pub fn can_transition_to(&self, target: &ReleaseStatus) -> bool {
        match (self, target) {
            (ReleaseStatus::Created, ReleaseStatus::Progressing) => true,
            (ReleaseStatus::Progressing, ReleaseStatus::Paused) => true,
            (ReleaseStatus::Progressing, ReleaseStatus::Promoted) => true,
            (ReleaseStatus::Progressing, ReleaseStatus::RolledBack) => true,
            (ReleaseStatus::Progressing, ReleaseStatus::Failed) => true,
            (ReleaseStatus::Paused, ReleaseStatus::Progressing) => true,
            (ReleaseStatus::Paused, ReleaseStatus::RolledBack) => true,
            _ => false,
        }
    }
}
...
impl ReleaseStage {
    pub fn percentage(&self) -> u8 {
        match self {
            ReleaseStage::Zero => 0,
            ReleaseStage::Five => 5,
            ReleaseStage::Twenty => 20,
            ReleaseStage::Fifty => 50,
            ReleaseStage::Hundred => 100,
        }
    }

    pub fn next(&self) -> Option<Self> {
        match self {
            ReleaseStage::Zero => Some(ReleaseStage::Five),
            ReleaseStage::Five => Some(ReleaseStage::Twenty),
            ReleaseStage::Twenty => Some(ReleaseStage::Fifty),
            ReleaseStage::Fifty => Some(ReleaseStage::Hundred),
            ReleaseStage::Hundred => None,
        }
    }

    pub fn prev(&self) -> Option<Self> {
        match self {
            ReleaseStage::Zero => None,
            ReleaseStage::Five => Some(ReleaseStage::Zero),
            ReleaseStage::Twenty => Some(ReleaseStage::Five),
            ReleaseStage::Fifty => Some(ReleaseStage::Twenty),
            ReleaseStage::Hundred => Some(ReleaseStage::Fifty),
        }
    }
...
impl Default for CanaryStrategy {
    fn default() -> Self {
        CanaryStrategy::TimeBasedProgressive {
...
impl std::fmt::Display for CanaryError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            CanaryError::InvalidStateTransition(msg) => write!(f, "Invalid state transition: {}", msg),
            CanaryError::ReleaseNotFound(id) => write!(f, "Release not found: {}", id),
            CanaryError::ReleaseAlreadyTerminal(id) => write!(f, "Release is already in terminal state: {}", id),
            CanaryError::NoMoreStages(msg) => write!(f, "No more stages: {}", msg),
            CanaryError::InvalidStage(msg) => write!(f, "Invalid stage: {}", msg),
        }
    }
}
...
impl CanaryManager {
    ...
    pub fn start_release(&mut self, release_id: Uuid) -> Result<&CanaryRelease, CanaryError> {
        ...
        release.update_traffic_shifts(target_stage.percentage() as f64);
