use crate::domain::dependency_graph::{DependencyGraph, NodeId, NodeType};
...
impl ChangeType {
    pub fn label(&self) -> &str {
        match self {
            ChangeType::CodeChange => "Code Change",
            ChangeType::ConfigChange => "Config Change",
            ChangeType::SchemaChange => "Schema Change",
            ChangeType::DependencyUpgrade => "Dependency Upgrade",
            ChangeType::Infrastructure => "Infrastructure",
        }
    }

    pub fn impact_multiplier(&self) -> f64 {
        match self {
            ChangeType::CodeChange => 1.0,
            ChangeType::ConfigChange => 0.8,
            ChangeType::SchemaChange => 1.5,
            ChangeType::DependencyUpgrade => 1.2,
            ChangeType::Infrastructure => 1.3,
        }
    }
}
...
impl ChangePriority {
    pub fn label(&self) -> &str {
        match self {
            ...
            ChangePriority::Normal => "Normal",
            ChangePriority::High => "High",
            ChangePriority::Urgent => "Urgent",
        }
    }
}
        ...
        for imp_id in &analysis.impacted_nodes {
            ...
            if criticality >= threshold {
                ...
                high_risk.push(ImpactNode {
                    id: imp_id.clone(),
                    name: node.map(|n| n.name.clone()).unwrap_or_else(|| imp_id.0.clone()),
                    node_type: node.map(|n| n.node_type.clone()).unwrap_or(NodeType::Module),
                    distance: dist,
...
mod tests {
    use super::*;
    use crate::domain::dependency_graph::{DependencyEdge, DependencyGraph, DependencyNode, EdgeType};
    ...
    fn test_schema_change_higher_impact() {
        ...
        let code_score: f64 = code_scope
            .direct_impact
            .iter()
            .map(|n| n.impact_score)
            .sum();
        let schema_score: f64 = schema_scope
            .direct_impact
            .iter()
            .map(|n| n.impact_score)
            .sum();

        assert!(schema_score > code_score);
    }
