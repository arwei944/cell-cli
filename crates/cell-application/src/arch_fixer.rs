use crate::arch_service::{ArchitectureRules, Violation};
...
impl FixType {
    pub fn label(&self) -> &str {
        match self {
            Self::ImportRemove => "移除导入",
            Self::ImportComment => "注释导入",
            Self::ImportRewrite => "重写导入",
            Self::FileMove => "移动文件",
            Self::CodeRefactor => "代码重构",
            Self::TodoMark => "标记TODO",
        }
    }
}
...
impl ArchitectureFixer {
    ...
    fn generate_fix_line(&self, line: &str, from_layer: &str, to_layer: &str, file: &str) -> (String, String, FixType) {
        ...
        if from_idx > to_idx {
            (
                ...
                format!("标记违规: {from_layer} 层 → {to_layer} 层依赖"),
                FixType::TodoMark,
            )
        } else if line.trim().starts_with("use ") || line.trim().starts_with("pub use ") {
            (
                format!("// TODO: 需要移至正确层 - {}", line.trim()),
                format!("标记待迁移: {from_layer} → {to_layer} (文件位于: {file})"),
                FixType::ImportComment,
            )
        } else {
            (
                line.to_string(),
                format!("需要手动重构: {from_layer} 不能依赖 {to_layer}"),
    ...
    fn generate_suggestions(&self, project_path: &str, violations: &[Violation]) -> Vec<FixSuggestion> {
        ...
        for (module, count) in &layer_violations {
            if *count >= 3 {
                suggestions.push(FixSuggestion {
                    file: module.to_string(),
                    line: None,
                    title: format!("模块 '{module}' 存在大量架构违规"),
                    description: format!("该模块有 {count} 个跨层依赖违规，建议重构到正确的层级"),
