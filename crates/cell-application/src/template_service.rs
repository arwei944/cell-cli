...
impl TemplateCategory {
    pub fn label(&self) -> &str {
        match self {
            Self::Basic => "基础项目",
            Self::CrudService => "CRUD服务",
            Self::Microservice => "微服务",
            Self::EventDriven => "事件驱动",
            Self::CliTool => "CLI工具",
            Self::Library => "类库",
            Self::FullStack => "全栈应用",
            Self::Custom => "自定义",
...
impl TemplateService {
    ...
    pub fn apply_template(&self, project_path: &str, template_id: &str, variables: HashMap<String, String>, force: bool) -> CellResult<ApplyResult> {
        let template = self.get_template(template_id)
            .ok_or_else(|| cell_domain::errors::CellError::Config(format!("Template '{template_id}' not found")))?;
        ...
        for file in &template.files {
            ...
            match std::fs::write(&target_path, &rendered_content) {
                Ok(()) => files_created.push(rendered_path),
