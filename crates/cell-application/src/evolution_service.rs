...
use cell_domain::evolution::{EvolutionLog, EvolutionPhase, IssueCategory, IssueSeverity, ImprovementCategory, ImpactLevel, EffortEstimate, Improvement, suggest_improvements_based_on_issues, ImprovementStatus, IssueStatus};
...
impl<T: EvolutionStorePort> EvolutionService<T> {
    ...
    pub fn apply_improvement(&self, project_path: &str, improvement_id: &str, applied_by: Option<&str>) -> CellResult<EvolutionLog> {
        ...
        let idx = log.improvements.iter().position(|i| i.id == id)
            .ok_or_else(|| CellError::Config(format!("Improvement with id {improvement_id} not found")))?;

        log.improvements[idx].status = ImprovementStatus::Applied;
        log.improvements[idx].applied_at = Some(chrono::Utc::now());
        log.improvements[idx].applied_by = applied_by.map(std::string::ToString::to_string);
...
IssueCategory::ProcessEfficiency | IssueCategory::Testing) |
(ImprovementCategory::NewFeature | ImprovementCategory::Automation,
...
fn scan_project_for_issues(project_path: &str) -> Vec<(String, String, IssueCategory, IssueSeverity)> {
    ...
    if file_count > 10 && !has_tests {
        issues.push((
            "测试覆盖率不足".to_string(),
            format!("项目有 {file_count} 个文件但缺少测试，容易引入回归bug。建议添加单元测试和架构测试。"),
    ...
    if total_lines > 2000 {
        issues.push((
            ...
            format!("项目规模较大（{total_lines} 行），需要确保熵值计算性能优化。建议使用缓存和增量计算。"),
    ...
    if file_count > 20 {
        issues.push((
            ...
            format!("项目文件数达到 {file_count}，建议配置增量编译和快速验证机制提升开发效率。"),
            IssueCategory::ProcessEfficiency,
            IssueSeverity::Medium,
        ));
    }
