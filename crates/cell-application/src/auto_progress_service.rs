...
use cell_domain::progress::{EventType, ProgressLog, ProgressStatus, NextStep};
...
impl AutoProgressTracker {
    ...
    fn get_git_state(&self, project_path: &str) -> CellResult<GitState> {
        ...
        let commit_hash = self.run_git_command(project_path, &["rev-parse", "HEAD"]).map_or_else(|_| "unknown".to_string(), |s| s.chars().take(8).collect());
        ...
        let timestamp = chrono::DateTime::parse_from_rfc3339(&timestamp_str).map_or_else(|_| Utc::now(), |dt| dt.with_timezone(&Utc));
    ...
    fn run_git_command(&self, project_path: &str, args: &[&str]) -> CellResult<String> {
        ...
        let output = Command::new("git")
            ...
            .output()
            .map_err(|e| cell_domain::errors::CellError::Other(format!("Git command failed: {e}")))?;

        if output.status.success() {
            Ok(String::from_utf8_lossy(&output.stdout).trim().to_string())
        } else {
            Err(cell_domain::errors::CellError::Other(
    ...
    ) -> CellResult<(ProgressStatus, Vec<cell_domain::progress::Blocker>, Vec<NextStep>)> {
    ...
    fn infer_task_name(&self, git_state: &GitState) -> String {
        ...
        if msg.contains("feat:") {
            msg.split("feat:").nth(1).map_or_else(|| "新功能开发".to_string(), |s| s.trim().to_string())
        } else if msg.contains("fix:") {
            msg.split("fix:").nth(1).map_or_else(|| "修复问题".to_string(), |s| s.trim().to_string())
        } else if msg.contains("refactor:") {
            msg.split("refactor:").nth(1).map_or_else(|| "代码重构".to_string(), |s| s.trim().to_string())
