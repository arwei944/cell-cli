...
impl RCAV2Service {
    ...
    pub fn get_analysis_detail(&self, id: &str) -> CellResult<AnalysisRecord> {
        self.analyses
            .get(id)
            .cloned()
            .ok_or_else(|| cell_domain::errors::CellError::NotFound(format!("Analysis {id} not found")))
    }
...
mod tests {
    ...
    fn test_analyze_signal_no_root_cause() {
        ...
        assert!(result.result.root_cause.is_none());
    }
    ...
    fn test_get_analysis_detail_not_found() {
        ...
        assert!(matches!(result, Err(cell_domain::errors::CellError::NotFound(_))));
    }
}