...
impl SimplicityChecker {
    ...
    pub fn check(&self, root: &str) -> CellResult<SimplicityReport> {
        ...
        if !src.exists() {
            return Err(cell_domain::errors::CellError::NotFound("src directory not found".into()));
        }
...
fn compute_file_score(
    ...
    score = (fns.iter().filter(|f| f.lines > checker.max_fn_lines).count() as f64).mul_add(-4.0, score);
    score = (fns.iter().filter(|f| f.complexity > checker.max_complexity).count() as f64).mul_add(-3.0, score);
    score = (fns.iter().filter(|f| f.nesting > checker.max_nesting).count() as f64).mul_add(-3.0, score);
    score = (fns.iter().filter(|f| f.args > checker.max_args).count() as f64).mul_add(-2.0, score);
    score = (fns.iter().filter(|f| f.clone_count > checker.max_clones_per_fn).count() as f64).mul_add(-2.0, score);
    score = (structs.iter().filter(|s| s.field_count > checker.max_struct_fields).count() as f64).mul_add(-2.0, score);

    let magic_count = issues.iter().filter(|i| i.category == Category::MagicNumber).count();
    score = (magic_count as f64).mul_add(-1.0, score);
    let unwrap_count = issues.iter().filter(|i| i.category == Category::UnwrapUsage).count();
    score = (unwrap_count as f64).mul_add(-1.5, score);
    if comment_ratio < checker.min_comment_ratio && total > 100 {
        score -= 5.0;
    }
    let todo_count = issues.iter().filter(|i| i.category == Category::TodoMarker).count();
    if todo_count > checker.max_todos {
        score = ((todo_count - checker.max_todos) as f64).mul_add(-0.5, score);
    }
    score
}
