...
use cell_domain::plugin_system::PluginManifest;
use cell_domain::plugin_validator::{
...
impl PluginValidatorService {
                ...
                .filter_map(std::result::Result::ok)
            {
                ...
                if file_path.is_file()
                    && file_path.file_name().and_then(|n| n.to_str())
                        == Some("plugin.json")
                    && let Ok(manifest) = Self::load_manifest(file_path.to_str().unwrap()) {
                        manifests.push(manifest);
                    }
            }
        }
    ...
    fn generate_audit_summary(&self, report: &ValidationReport) -> AuditSummary {
        ...
        if critical_count > 0 {
            recommendations.push(format!("Fix {critical_count} Critical issues"));
        }
        if error_count > 0 {
            recommendations.push(format!("Fix {error_count} Error issues"));
        }
        if warning_count > 0 {
            recommendations.push(format!("Review {warning_count} Warning issues"));
        }
    ...
    pub fn format_audit_result(&self, result: &PluginAuditResult) -> String {
        ...
        output.push_str(&format!("Risk Level: {risk_label}\n"));
...
mod tests {
    use super::*;
    use cell_domain::plugin_system::{Permission, PluginId, PluginKind, PluginManifest};
    use std::fs;
    use std::fs::File;
    use std::io::Write;
    ...
    fn test_validate_plugin_invalid() {
        ...
        manifest.id = PluginId(String::new());
        let manifest_path = dir.path().join("plugin.json");
        write_manifest_to_file(&manifest_path, &manifest);
    ...
    fn test_audit_plugin() {
        ...
        bad_manifest.id = PluginId(String::new());
        bad_manifest.version = "invalid".into();
        write_manifest_to_file(&bad_dir.join("plugin.json"), &bad_manifest);

        let good_dir = dir.path().join("good-plugin");
        fs::create_dir_all(&good_dir).unwrap();
        let good_manifest = create_test_manifest("good-plugin", "Good Plugin");
        write_manifest_to_file(&good_dir.join("plugin.json"), &good_manifest);
