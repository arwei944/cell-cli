use cell_domain::canary_release::{CanaryManager, CanaryRelease, CanaryStrategy, CanaryError};
...
impl CanaryService {
    ...
    pub fn create_canary(
        ...
        if self.name_to_id.contains_key(&name) {
            return Err(cell_domain::errors::CellError::Validation(
                format!("Canary release '{name}' already exists"),
            ));
        }
    ...
    pub fn promote_canary(&mut self, name: impl AsRef<str>) -> CellResult<CanaryStatus> {
        ...
        let release = self.manager.promote_release(*id)
            .map_err(cell_error_from_canary_error)?;
        Ok(release.into())
    }
...
fn cell_error_from_canary_error(e: CanaryError) -> CellError {
    match e {
        ...
        CanaryError::InvalidStateTransition(msg)
        | CanaryError::ReleaseAlreadyTerminal(msg)
        | CanaryError::NoMoreStages(msg)
