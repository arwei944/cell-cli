...
use cell_domain::service_mesh::MeshGenerator;
...
impl ServiceMeshService {
    ...
    pub fn generate_istio_config_with_options(
        ...
        let versions_str: Vec<String> = versions.iter().map(std::string::ToString::to_string).collect();
        let generator = MeshGenerator::new();
        let config = generator.generate_complete_config(name, namespace, &versions_str, gateway_name.map(std::string::ToString::to_string));
        ...
        Ok(IstioConfig {
            ...
            gateway_name: gateway_name.map(std::string::ToString::to_string),
            yaml,
    ...
    pub fn validate_istio_config_content(&self, content: &str) -> CellResult<ValidationResult> {
        ...
        let yaml_value: serde_yaml::Value = serde_yaml::from_str(content)
            .map_err(|e| {
                ...
                cell_domain::errors::CellError::Yaml(e)
            })?;
        ...
        for doc in docs {
            if let serde_yaml::Value::Mapping(m) = doc {
                for (k, v) in m {
                    ...
                    match key {
                        "kind" => {
                            kind = v.as_str().map(std::string::ToString::to_string);
                        ...
                        "metadata" => {
                            if let serde_yaml::Value::Mapping(meta) = v {
                                for (mk, mv) in meta {
                                    ...
                                    match mkey {
                                        "name" => name = mv.as_str().map(std::string::ToString::to_string),
                                        "namespace" => namespace = mv.as_str().map(std::string::ToString::to_string),
                                        _ => {}
                                    }
                                }
                            }
    ...
    pub fn format_validation_result(&self, result: &ValidationResult) -> String {
        ...
        if let Some(kind) = &result.kind {
            output.push_str(&format!("资源类型: {kind}\n"));
        }
        if let Some(name) = &result.name {
            output.push_str(&format!("名称: {name}\n"));
        }
        if let Some(namespace) = &result.namespace {
            output.push_str(&format!("命名空间: {namespace}\n"));
        }
