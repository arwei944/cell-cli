...
impl ConfigService {
    ...
    pub fn load(&self) -> CellResult<CellConfig> {
        ...
        let content = std::fs::read_to_string(path)
            .map_err(cell_domain::errors::CellError::Io)?;

        let config: CellConfig = toml::from_str(&content)
            .map_err(|e| cell_domain::errors::CellError::Config(format!("Failed to parse config: {e}")))?;
    ...
    pub fn save(&self, config: &CellConfig) -> CellResult<()> {
        ...
        let content = toml::to_string_pretty(config)
            .map_err(|e| cell_domain::errors::CellError::Config(format!("Failed to serialize config: {e}")))?;
    ...
    fn set_value(&self, config: &mut CellConfig, key: &str, value: &str) -> CellResult<()> {
        ...
        let mut config_value = serde_json::to_value(&*config)
            .map_err(|e| cell_domain::errors::CellError::Config(format!("{e}")))?;
        ...
        if let Some(obj) = config_value.get_mut(section).and_then(|v| v.as_object_mut()) {
            ...
            *config = serde_json::from_value(config_value)
                .map_err(|e| cell_domain::errors::CellError::Config(format!("Invalid value: {e}")))?;

            return Ok(());
        }

        Err(cell_domain::errors::CellError::Config(
    ...
    pub fn format_show(&self, config: &CellConfig) -> String {
        ...
        output.push_str(&format!("  │  描述:       {desc:<37}│\n"));
        output.push_str("  └─────────────────────────────────────────────────────┘\n\n");
