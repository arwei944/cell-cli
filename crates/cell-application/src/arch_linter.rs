            ...
            if from == from_rule && to == to_rule {
                if self.is_rule_enabled(rule_id) {
                    violations.push(LintViolation {
                        ...
                        suggestion: Some(format!("移除 {from} → {to} 的依赖")),
                    });
                }
                break;
            }
                ...
                && !in_function
            {
                ...
                if param_count > 6 && self.is_rule_enabled("C004") {
                    violations.push(LintViolation {
                        ...
                        line: line_num + 1,
                        message: format!(
                            "函数 `{func_name}` 参数过多 ({param_count})，建议 ≤ 6 个"
                        ),
                ...
                if brace_count <= 0 && func_start > 0 {
                    ...
                    if func_length > 80 && self.is_rule_enabled("C001") {
                        violations.push(LintViolation {
                            ...
                            line: func_start,
                            message: format!(
                                "函数 `{func_name}` 过长 ({func_length} 行)，建议 ≤ 80 行"
                            ),
                    ...
                    if max_nesting > 4 && self.is_rule_enabled("C003") {
                        violations.push(LintViolation {
                            ...
                            line: func_start,
                            message: format!(
                                "函数 `{func_name}` 嵌套过深 ({max_nesting} 层)，建议 ≤ 4 层"
                            ),
    ...
    fn check_port_stability(&self, file: &str, content: &str, violations: &mut Vec<LintViolation>) {
        ...
        if method_count > 10 {
            violations.push(LintViolation {
                ...
                file: file.to_string(),
                line: 1,
                message: format!("Port 接口方法过多 ({method_count} 个)，可能影响契约稳定性"),
    ...
    fn check_deletability(&self, file: &str, content: &str, violations: &mut Vec<LintViolation>) {
        ...
        if pub_items > 15 && imports > 10 {
            violations.push(LintViolation {
                ...
                file: file.to_string(),
                line: 1,
                message: format!("模块公共接口较多 ({pub_items}) 且内部依赖广泛 ({imports})，可能影响可删除性"),
    ...
    pub fn format_result(&self, result: &LintResult) -> String {
        ...
        if !result.by_category.is_empty() {
            ...
            for (cat, count) in cats {
                output.push_str(&format!("  {cat}: {count} 个\n"));
            }
            output.push('\n');
        }
        ...
        if result.violations.is_empty() {
            ...
            for v in &sorted {
                ...
                if let Some(suggestion) = &v.suggestion {
                    output.push_str(&format!("     💡 建议: {suggestion}\n"));
                }
            }
        }
...
",
    ...
    fn test_valid_architecture_no_errors() {
        ...
        assert!(
            ...
            "Valid architecture should have no layer errors: {layer_errors:?}"
        );
    }
