use crate::arch_service::{ArchitectureRules, ValidationResult, Severity};
...
impl ArchitectureAdvisor {
        ...
        for (layer_name, stats) in &validation.layer_stats {
            if stats.violations > 0 {
                advices.push(Advice {
                    ...
                    title: format!("{layer_name}层存在架构违规"),
                    description: format!("{}层有{}个依赖违规，违反了分层架构原则", layer_name, stats.violations),
                    suggestion: format!("检查{layer_name}层的import语句，确保只依赖内层（domain -> application -> adapters -> interfaces）"),
            ...
            if stats.external_deps > stats.internal_deps * 2 {
                advices.push(Advice {
                    ...
                    title: format!("{layer_name}层外部依赖过多"),
    ...
    fn generate_entropy_advices(&self, entropy: &cell_domain::entropy::EntropyReport) -> Vec<Advice> {
        let mut advices = Vec::new();
    ...
    pub fn format_result(&self, result: &AdviceResult) -> String {
        ...
        if !result.advices.is_empty() {
            ...
            for advice in &result.advices {
                ...
                output.push_str(&format!("   {}\n", advice.description));
                output.push_str(&format!("   💡 {}\n", advice.suggestion));
                output.push_str(&format!("   {effort_label} · {impact_icon}\n"));
