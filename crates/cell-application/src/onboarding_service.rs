...
"#;
    ...
    fn register_agent(&self, project_path: &str) -> CellResult<OnboardingStepResult> {
        ...
        let agent_file = agents_dir.join(format!("{agent_id}.json"));
        ...
        Ok(OnboardingStepResult {
            ...
            success: true,
            skipped: false,
            message: format!("Agent 已注册: {agent_id} (Developer)"),
        })
    }
    ...
    pub fn generate_ready_report(&self, project_path: &str) -> CellResult<ReadyReport> {
        ...
        let suggestions = vec![
            "查看项目架构: cell arch status".to_string(),
            "查看待处理任务: cell task list".to_string(),
            "开始新任务: cell dev start <任务名>".to_string(),
        ];
    ...
    pub fn format_doctor_report(&self, report: &OnboardingReport) -> String {
        ...
        for check in &report.checks {
            ...
            output.push_str(&format!("    状态: {status}\n"));
            if let Some(hint) = &check.install_hint
                && !check.installed {
                    output.push_str(&format!("    提示: {hint}\n"));
                }
        }
