...
use crate::entropy_trend_service::EntropyTrendService;
use crate::fast_verify_service::FastVerifyService;
use crate::ports::decision_store::DecisionStorePort;
use crate::ports::evolution_store::EvolutionStorePort;
use crate::ports::handoff_exporter::HandoffExporterPort;
...
use cell_domain::evolution::{IssueCategory, IssueSeverity};
use cell_domain::progress::EventType;
use crate::progress_service::ProgressService;
use crate::handoff_service::HandoffService;
use crate::evolution_service::EvolutionService;
use crate::decision_service::DecisionService;
...
impl DevPhase {
    ...
    pub fn next(&self) -> Option<Self> {
        match self {
            Self::Starting => Some(Self::Designing),
            Self::Designing => Some(Self::Coding),
            Self::Coding => Some(Self::Verifying),
            Self::Verifying => Some(Self::HandingOff),
            Self::HandingOff => Some(Self::Complete),
            Self::Complete => None,
        }
    }
}
    ...
    pub fn design_phase(&self, project_path: &str) -> CellResult<PhaseResult> {
        ...
        match trend_service.analyze(project_path) {
            Ok(trend) => {
                ...
                if trend.trend == crate::entropy_trend_service::TrendDirection::Increasing {
        ...
        if let Some(baseline) = state.baseline_entropy {
            match entropy_service::run_entropy_check(project_path) {
                Ok(current) => {
                    ...
                    if diff >= 5.0 {
                        warnings.push(format!("熵值上升 {diff:.2}，注意控制复杂度"));
                    }
                }
                Err(e) => {
