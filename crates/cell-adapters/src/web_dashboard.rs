use cell_application::auto_progress_service::{AutoProgressTracker, AutoProgressSnapshot};
use crate::file_decision_store::FileDecisionStore;
use crate::file_evolution_store::FileEvolutionStore;
use crate::file_progress_store::FileProgressStore;
...
use cell_application::observability::{build_dashboard, ObservabilityService};
...
use cell_domain::observability::DashboardData;
...
pub async fn start_dashboard_server(project_path: &str, port: u16) -> CellResult<()> {
    ...
    let listener = tokio::net::TcpListener::bind(addr)
        .await
        .map_err(|e| cell_domain::errors::CellError::Config(format!("绑定端口失败: {e}")))?;
    axum::serve(listener, app)
        .await
        .map_err(|e| cell_domain::errors::CellError::Config(format!("服务运行失败: {e}")))?;
