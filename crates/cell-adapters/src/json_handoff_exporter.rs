...
impl HandoffExporterPort for FileHandoffExporter {
    fn export_json(&self, package: &HandoffPackage, output_path: &str) -> CellResult<String> {
        ...
        let content = serde_json::to_string_pretty(package).map_err(|e| cell_domain::errors::CellError::Other(e.to_string()))?;
    ...
    fn import_json(&self, path: &str) -> CellResult<HandoffPackage> {
        ...
        let package: HandoffPackage = serde_json::from_str(&content).map_err(|e| cell_domain::errors::CellError::Other(e.to_string()))?;
        Ok(package)
    }
}
