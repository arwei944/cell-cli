...
use cell_application::entropy_service::run_entropy_check;
use cell_application::fast_verify_service::FastVerifyService;
use cell_application::handoff_service::HandoffService;
use cell_application::progress_bar::StepProgress;
use cell_application::progress_service::ProgressService;
use cell_domain::errors::{CellError, CellResult};
use cell_domain::progress::EventType;
use crate::cli::{HandoffArgs, HandoffSub, ProgressArgs, ProgressSub};
use std::path::Path;
...
pub fn cmd_handoff(args: HandoffArgs) -> CellResult<()> {
    ...
    let handoff_service = HandoffService::new(exporter, progress_store, decision_store);
...
fn handle_handoff_generate(
    ...
    let project_name = name.unwrap_or_else(|| {
        std::env::current_dir().ok()
            .and_then(|p| p.file_name().and_then(|n| n.to_str().map(std::string::ToString::to_string)))
...
fn export_handoff(
    handoff_service: &HandoffService<FileHandoffExporter, FileProgressStore, FileDecisionStore>,
    pkg: &cell_domain::handoff::HandoffPackage,
    out_dir: &str,
    base_name: &str,
    ext: &str,
) -> CellResult<()> {
    let path = Path::new(out_dir).join(format!("{base_name}.{ext}"));
    let path_str = path.to_str().ok_or_else(|| CellError::Config(format!("Invalid {ext} path")))?;
    if ext == "json" {
...
fn print_handoff_result(
    ...
    println!();
    if !no_json { println!("📄 JSON 交接包: {out_dir}/{base_name}.json"); }
    if !no_md { println!("📝 Markdown 交接包: {out_dir}/{base_name}.md"); }
...
fn handle_handoff_commit(
    ...
    if quick {
        ...
        if !result.passed {
            progress.fail_current("快速验证失败");
            std::process::exit(1);
        }
    } else {
        let result = verify_service.deep_check(".")?;
        if !result.passed {
            progress.fail_current("深度验证失败");
            std::process::exit(1);
        }
    }
    progress.complete_current();
...
pub fn cmd_progress(args: ProgressArgs) -> CellResult<()> {
    ...
    match args.sub {
        ...
        ProgressSub::Next { description, priority, minutes } => {
            ...
            println!("📝 已添加下一步: {description} (优先级: {priority})");
            println!("   ID: {step_id}");
        }
        ProgressSub::Done { id } => {
            ...
            if !log.blockers.is_empty() {
                ...
                for b in &log.blockers {
                    let icon = match b.status {
                        cell_domain::progress::BlockerStatus::Active => "🔴",
                        cell_domain::progress::BlockerStatus::Resolved => "🟢",
                        cell_domain::progress::BlockerStatus::Bypassed => "🟡",
                    };
...
fn parse_event_type(s: &str) -> CellResult<EventType> {
    match s.to_lowercase().as_str() {
        ...
        _ => Err(CellError::Config(format!(
            "Unknown event type: {s}. Valid: start, update, decision, blocker, note, complete"
        ))),
    }
}
