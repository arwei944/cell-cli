use cell_application::pattern_library_service::PatternLibraryService;
use cell_domain::errors::CellResult;
use crate::cli::{PatternArgs, PatternSub};
        ...
        PatternSub::Recommend { id } => {
            ...
            if let Some(pattern_id) = id {
                ...
                let related: Vec<_> = detail.related_patterns.iter()
                    .map(|(p, reason)| cell_application::pattern_library_service::PatternRecommendation {
                        id: p.id.clone(),
                        name: p.name.clone(),
