...
use cell_application::workflow_engine::WorkflowEngine;
use cell_domain::errors::CellResult;
use cell_domain::workflow::{WorkflowCommand, WorkflowAction, AgentInfo, GateResult, WorkflowResponse};
use crate::cli::{WorkflowArgs, WorkflowSub};
    ...
    let response = match args.sub {
        ...
        WorkflowSub::Exec { command: cmd } => {
            let workflow_cmd: WorkflowCommand = serde_json::from_str(&cmd)
                .map_err(|e| cell_domain::errors::CellError::Config(format!("Invalid command JSON: {e}")))?;
...
fn print_text_response(response: &WorkflowResponse) {
    ...
    if let Some(data) = &response.data {
        ...
        if let Some(all_passed) = data.get("all_gates_passed").and_then(serde_json::Value::as_bool)
            && all_passed {
                println!("\n  🎉 所有门禁已通过，可以推进到下一阶段！");
            }
        ...
        if let Some(next) = data.get("next_phase").and_then(|v| v.as_str()) {
            println!("\n  下一阶段: {next}");
            if let Some(can_advance) = data.get("can_advance").and_then(serde_json::Value::as_bool) {
                if can_advance {
                    println!("  可推进: ✅ 是");
                } else {
