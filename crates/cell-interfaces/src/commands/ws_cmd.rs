use cell_application::websocket_dashboard_service::WebSocketDashboardService;
use cell_domain::errors::CellResult;
use crate::cli::{WsArgs, WsSub};
...
pub fn cmd_ws(args: WsArgs) -> CellResult<()> {
    ...
    match args.sub {
        WsSub::Serve { port } => {
            ...
            println!("  端口: {port}");
            println!("  WebSocket: ws://localhost:{port}/ws/dashboard");
            println!("  HTML: http://localhost:{port}/dashboard");
            println!();
        ...
        WsSub::Test {} => {
            ...
            let rt = tokio::runtime::Runtime::new()
                .map_err(|e| cell_domain::errors::CellError::Config(e.to_string()))?;
            ...
            rt.block_on(async {
                ...
                Ok::<(), cell_domain::errors::CellError>(())
            })?;
