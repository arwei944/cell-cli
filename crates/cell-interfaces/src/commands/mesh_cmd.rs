use cell_application::service_mesh_service::ServiceMeshService;
use cell_domain::errors::CellResult;
use crate::cli::{MeshArgs, MeshSub};
...
pub fn cmd_mesh(args: MeshArgs) -> CellResult<()> {
    ...
    match args.sub {
        MeshSub::Generate { name, namespace, versions, gateway, output } => {
            ...
            let vers: Vec<&str> = versions_vec.iter().map(std::string::String::as_str).collect();
