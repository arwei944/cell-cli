use cell_application::enforcement_service::{EnforcementService, PolicyLevel, EnforcementReport};
use cell_application::ci_template_service::CiTemplateService;
use cell_domain::errors::CellResult;
use crate::cli::{EnforcementArgs, EnforcementSub, CiSub};
    ...
    match args.sub {
        ...
        EnforcementSub::SetPolicy { policy, level } => {
            let level_enum = match level.to_lowercase().as_str() {
                ...
                _ => return Err(cell_domain::errors::CellError::Config(
                    format!("Invalid level: {level}. Use allow, warn, or block.")
                )),
            };
            ...
            match sub {
                ...
                CiSub::Apply { provider } => {
                    ...
                    let provider_enum = match provider_lower.as_str() {
                        "github" => cell_application::enforcement_service::CiProvider::Github,
                        "gitlab" => cell_application::enforcement_service::CiProvider::Gitlab,
                        "jenkins" => cell_application::enforcement_service::CiProvider::Jenkins,
                        "gitee" => cell_application::enforcement_service::CiProvider::Gitee,
                        _ => return Err(cell_domain::errors::CellError::Config(
...
fn print_ci_template(template: &cell_application::ci_template_service::CiTemplate) {
