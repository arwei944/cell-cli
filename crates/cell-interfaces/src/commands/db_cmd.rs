use cell_application::db_migration_service::{DbMigrationService, ChangeType, MigrationStatus};
use cell_domain::errors::CellResult;
use crate::cli::{DbArgs, DbSub};
...
pub fn cmd_db(args: DbArgs) -> CellResult<()> {
    ...
    match args.sub {
        DbSub::Create { name, description } => {
            let changes = vec![
                cell_application::db_migration_service::SchemaChange {
