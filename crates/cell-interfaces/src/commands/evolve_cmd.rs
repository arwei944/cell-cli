...
fn show_evolve_status(service: &EvolutionService<FileEvolutionStore>) -> CellResult<()> {
    if let Some(log) = service.get_current_cycle(".")? {
...
fn print_evolve_issues(issues: &[cell_domain::evolution::Issue]) {
...
fn print_evolve_improvements(improvements: &[cell_domain::evolution::Improvement]) {
    if improvements.is_empty() { return; }
    println!("\n💡 改进建议:");
    for imp in improvements {
        let status_icon = match imp.status {
            cell_domain::evolution::ImprovementStatus::Proposed => "📝",
            cell_domain::evolution::ImprovementStatus::Planned => "📋",
            cell_domain::evolution::ImprovementStatus::InProgress => "🔧",
            cell_domain::evolution::ImprovementStatus::Applied => "✅",
            cell_domain::evolution::ImprovementStatus::Verified => "✅",
            cell_domain::evolution::ImprovementStatus::Rejected => "❌",
            cell_domain::evolution::ImprovementStatus::RolledBack => "↩️",
        };
...
fn parse_issue_category(s: &str) -> CellResult<IssueCategory> {
    match s.to_lowercase().as_str() {
        ...
        _ => Err(CellError::Config(format!(
            "Unknown issue category: {s}. Valid: process, tool, quality, handoff, codegen, docs, arch, entropy, test, other"
        ))),
    }
}
...
fn parse_issue_severity(s: &str) -> CellResult<IssueSeverity> {
    match s.to_lowercase().as_str() {
        ...
        _ => Err(CellError::Config(format!(
            "Unknown severity: {s}. Valid: critical, high, medium, low, trivial"
        ))),
    }
}
...
fn parse_improvement_category(s: &str) -> CellResult<ImprovementCategory> {
    match s.to_lowercase().as_str() {
        ...
        _ => Err(CellError::Config(format!(
            "Unknown improvement category: {s}. Valid: process, new-feature, quality-gate, template, docs, automation, refactoring"
        ))),
    }
}
...
fn parse_impact_level(s: &str) -> CellResult<ImpactLevel> {
    match s.to_lowercase().as_str() {
        ...
        _ => Err(CellError::Config(format!(
            "Unknown impact level: {s}. Valid: transformational, high, medium, low, minimal"
        ))),
    }
}
...
fn parse_effort_estimate(s: &str) -> CellResult<EffortEstimate> {
    match s.to_lowercase().as_str() {
        ...
        _ => Err(CellError::Config(format!(
            "Unknown effort estimate: {s}. Valid: minutes, hours, days, weeks"
        ))),
    }
}
