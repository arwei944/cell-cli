use cell_domain::errors::{CellResult, CellError};
use cell_domain::event_schema::{EventSchema, EventSchemaRegistry, EventField, EventFieldType};
use crate::cli::EventSub;
use std::fs;
use std::path::Path;

pub fn cmd_event(args: crate::cli::EventArgs) -> CellResult<()> {
    match args.sub {
...
fn show_schema(name: &str, version: Option<&str>) -> CellResult<()> {
    ...
    let schema = match version {
        Some(v) => registry.get_version(name, v)
            .ok_or_else(|| CellError::NotFound(format!(
                "Schema '{name}' version '{v}' not found"
            )))?,
    ...
    println!();
    println!("Fields:");
    println!("  {:<5} {:<25} {:<20} {:<8} Description", "Tag", "Name", "Type", "Required");
    println!("  {}", "-".repeat(85));
        ...
        .map_err(|e| CellError::NotFound(e.to_string()))?
    {
        ...
        if path.is_file() {
            ...
            if matches!(ext, Some("yaml" | "yml" | "json"))
                && let Ok(schema) = load_schema_from_file(path.to_str().unwrap()) {
