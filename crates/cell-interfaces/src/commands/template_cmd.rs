use cell_application::template_service::{TemplateCategory, TemplateService};
use cell_domain::errors::CellResult;
use crate::cli::{TemplateArgs, TemplateSub};
...
pub fn cmd_template(args: TemplateArgs) -> CellResult<()> {
    ...
    match args.sub {
        TemplateSub::List { category } => {
            ...
            for template in &templates {
                if current_category.as_ref() != Some(&template.category) {
                    ...
                    println!("  {} [{}]", template.category.label(), template.category);
                    println!("  {}", "─".repeat(50));
                }
        ...
        TemplateSub::Show { id } => {
            if let Some(template) = service.get_template(&id) {
                println!("\n📋 模板详情\n");
...
fn parse_category(s: &str) -> CellResult<TemplateCategory> {
    match s.to_lowercase().as_str() {
        ...
        _ => Err(cell_domain::errors::CellError::Config(format!(
            "Unknown category: {s}. Valid: basic, crud, microservice, event, cli, library, fullstack, custom"
        ))),
    }
}
