use cell_application::rule_engine_service::RuleEngineService;
use cell_domain::errors::CellResult;
use crate::cli::{RuleArgs, RuleSub};
    ...
    match args.sub {
        ...
        RuleSub::Evaluate { id, data } => {
            ...
            let rule_id = id.parse::<uuid::Uuid>().map_err(|e| {
                cell_domain::errors::CellError::Validation(format!("Invalid rule ID: {e}"))
            })?;
        ...
        RuleSub::Activate { id } => {
            ...
            println!("\n✅ Rule '{id}' activated successfully!");
