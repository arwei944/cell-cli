use cell_application::entropy_config_loader::EntropyConfigLoader;
use cell_application::entropy_service::run_entropy_check;
use cell_application::entropy_trend_service::EntropyTrendService;
use cell_application::incremental_entropy_service::IncrementalEntropyService;
use cell_domain::errors::CellResult;
use cell_domain::feature::{FeatureFlagConfig, FeatureFlagType};
use crate::cli::{EntropyArgs, EntropySub, EntropyConfigSub, FeatureArgs, FeatureSub, FlagAction};
use std::path::Path;
...
pub fn cmd_entropy(args: EntropyArgs, format: Option<String>) -> CellResult<()> {
    match args.sub {
        ...
        EntropySub::Config { sub } => {
            let loader = EntropyConfigLoader::load(".")?;
            match sub {
                EntropyConfigSub::Init { force } => {
                    let path = EntropyConfigLoader::init_config(".", force)?;
                    println!("✅ 已生成 entropy.yaml: {path}");
                }
                EntropyConfigSub::Show {} => {
                    println!("{}", loader.format_config());
                }
            }
        }
    }
...
fn output_entropy_report(report: &cell_domain::entropy::EntropyReport, format: Option<&str>) {
    match format {
        Some("json") => {
