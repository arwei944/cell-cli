    ...
    pub fn validate(&self) -> Result<(), SagaError> {
        ...
        for (i, step) in self.steps.iter().enumerate() {
            if step.name.is_empty() {
                return Err(SagaError::InvalidDefinition(format!("Step {i} has empty name")));
            }
...
impl fmt::Display for SagaError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InvalidDefinition(msg) => write!(f, "Invalid saga definition: {msg}"),
            Self::StepExecutionFailed(msg) => write!(f, "Step execution failed: {msg}"),
            Self::CompensationFailed(msg) => write!(f, "Compensation failed: {msg}"),
