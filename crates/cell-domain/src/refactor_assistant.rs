...
impl fmt::Display for RefactorType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ExtractFunction => write!(f, "ExtractFunction"),
            Self::ExtractClass => write!(f, "ExtractClass"),
            Self::ExtractInterface => write!(f, "ExtractInterface"),
            Self::SplitModule => write!(f, "SplitModule"),
            Self::MergeModule => write!(f, "MergeModule"),
            Self::MoveFile => write!(f, "MoveFile"),
            Self::ChangeSignature => write!(f, "ChangeSignature"),
        }
    }
}
...
impl fmt::Display for CodeSmellType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::LongFunction => write!(f, "LongFunction"),
            Self::GodClass => write!(f, "GodClass"),
            Self::DuplicateCode => write!(f, "DuplicateCode"),
            Self::LongParameterList => write!(f, "LongParameterList"),
            Self::DeepInheritance => write!(f, "DeepInheritance"),
            Self::ShotgunSurgery => write!(f, "ShotgunSurgery"),
            Self::FeatureEnvy => write!(f, "FeatureEnvy"),
...
impl fmt::Display for RefactorError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::PlanNotReady(msg) => write!(f, "Plan not ready: {msg}"),
        }
    }
}
...
impl RefactorAssistant {
    ...
    pub fn detect_code_smells(&self, target: &str, metrics: &HashMap<String, f64>) -> Vec<CodeSmell> {
        ...
        for detector in &self.smell_detectors {
            if let Some(value) = metrics.get(&detector.metric_name)
                && detector.operator.compare(*value, detector.threshold) {
                    let evidence = format!(
                        "{}: {:.2} (threshold: {:.2})",
    ...
    fn smell_to_refactor_type(&self, smell: &CodeSmellType) -> RefactorType {
        match smell {
            CodeSmellType::LongFunction | CodeSmellType::DuplicateCode => RefactorType::ExtractFunction,
            CodeSmellType::GodClass | CodeSmellType::DeepInheritance => RefactorType::ExtractClass,
    ...
    fn estimate_effort(&self, smell: &CodeSmellType) -> f64 {
        match smell {
            CodeSmellType::LongFunction | CodeSmellType::DataClass => 2.0,
    ...
    pub fn assess_impact(&self, proposal_id: &str) -> RefactorResult<RefactorImpact> {
        ...
        for file in &proposal.affected_files {
            ...
            affected_tests.push(format!("{file}_test"));
        }
    ...
    pub fn generate_execution_plan(&mut self, proposal_id: &str) -> RefactorResult<RefactorExecutionPlan> {
        ...
        steps.push(RefactorStep {
            id: format!("step-{proposal_id}-backup"),
            description: "备份原始文件".to_string(),
            order: step_counter as usize,
            file_path: proposal.affected_files.first().cloned().unwrap_or_default(),
        ...
        step_counter += 1;
        steps.push(RefactorStep {
            id: format!("step-{proposal_id}-apply"),
        ...
        step_counter += 1;
        steps.push(RefactorStep {
            id: format!("step-{proposal_id}-verify"),
