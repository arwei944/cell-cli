use crate::plugin_system::{Permission, PluginId, PluginManifest};
...
impl ValidationRule {
    ...
    pub fn check(&self, manifest: &PluginManifest) -> Option<ValidationFinding> {
        ...
        if finding.rule_id.is_empty() {
            finding.rule_id.clone_from(&self.id);
            finding.rule_name.clone_from(&self.name);
            finding.severity = self.severity;
            finding.validation_type.clone_from(&self.validation_type);
        }
        Some(finding)
    }
}
...
mod tests {
    use super::*;
    use crate::plugin_system::PluginKind;
    ...
    fn test_validation_report() {
        ...
        manifests[1].id = PluginId(String::new());
