...
impl ConfigEnvironment {
    pub fn parent(&self) -> Option<Self> {
        match self {
            Self::Dev => None,
            Self::Test => Some(Self::Dev),
            Self::Staging => Some(Self::Test),
...
impl ConfigKey {
    ...
    pub fn env_key(&self) -> String {
        self.service.as_ref().map_or_else(
            || format!("{:?}::{}", self.environment, self.key),
            |svc| format!("{:?}:{}:{}", self.environment, svc, self.key),
        )
    }
}
    ...
    pub fn new(
        ...
        if !value_type.validate(&value) {
            return Err(CellError::Validation(format!(
                "Value does not match type {value_type:?}"
            )));
        }
...
impl ConfigCenter {
    ...
    pub fn rollback(
        ...
        let target_entry = versions
            ...
            .ok_or_else(|| CellError::NotFound(format!("Version {target_version} not found")))?
            .entry
            .clone();
    ...
    pub fn get_version(&self, key: &ConfigKey, version: u64) -> CellResult<&ConfigEntry> {
        ...
        self.entries
            ...
            .map(|v| &v.entry)
            .ok_or_else(|| CellError::NotFound(format!("Version {version} not found")))
    }
}
...
mod tests {
    ...
    fn test_config_type_validation() {
        ...
        assert!(ConfigType::Float.validate(&json!(std::f64::consts::PI)));
