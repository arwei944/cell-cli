...
impl EntropyReport {
    pub fn to_pretty_text(&self) -> String {
        ...
        if top_n > 0 {
            lines.push("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━".to_string());
            lines.push(format!("  📁 Top {top_n} 最高熵值文件"));
            lines.push("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━".to_string());
...
impl EntropyGrade {
    pub fn from_score(score: f64) -> Self {
        match score {
            s if s < 20.0 => Self::Healthy,
            s if s < 40.0 => Self::Notice,
            s if s < 60.0 => Self::Warning,
            s if s < 80.0 => Self::Danger,
            _ => Self::Critical,
        }
    }
}
...
pub fn shannon_entropy(probabilities: &[f64]) -> f64 {
    ...
    for &p in probabilities {
        if p > 0.0 {
            let prob = p / total;
            entropy = prob.mul_add(-prob.log2(), entropy);
        }
    }
    entropy
}
...
pub fn calculate_complexity_for_file(content: &str) -> FileComplexityInfo {
    ...
    let nesting_penalty = if max_depth > 4 {
        (f64::from(max_depth - 4) * 10.0).min(25.0)
    } else {
        0.0
    };
    ...
    let cc_score = if cyclomatic_complexity <= 10 {
        ...
        ((cyclomatic_complexity - 10) as f64).mul_add(2.0, 10.0)
    } else {
        ((cyclomatic_complexity - 20) as f64).mul_add(1.5, 30.0)
    }.min(60.0);

    let overall_complexity = (depth_score.mul_add(0.3, cc_score * 0.4) + branch_score * 0.15 + nesting_penalty * 0.15)
        .clamp(0.0, 100.0);

    FileComplexityInfo {
        score: overall_complexity,
        cyclomatic_complexity,
        max_nesting_depth: usize::try_from(max_depth.max(0)).unwrap_or_default(),
        function_count,
        branch_count,
        loop_count,
        match_count,
...
pub fn calculate_naming_for_file(content: &str) -> f64 {
    ...
    for line in &lines {
        let trimmed = line.trim();
        if (trimmed.starts_with("fn ") || trimmed.contains(" fn "))
            && let Some(name) = extract_identifier(trimmed, "fn") {
                identifiers.push(name);
            }
        if (trimmed.starts_with("struct ") || trimmed.contains(" struct "))
            && let Some(name) = extract_identifier(trimmed, "struct") {
                identifiers.push(name);
            }
        if (trimmed.starts_with("enum ") || trimmed.contains(" enum "))
            && let Some(name) = extract_identifier(trimmed, "enum") {
                identifiers.push(name);
            }
        if (trimmed.starts_with("trait ") || trimmed.contains(" trait "))
            && let Some(name) = extract_identifier(trimmed, "trait") {
                identifiers.push(name);
            }
        if (trimmed.starts_with("let ") || trimmed.starts_with("let mut "))
            && let Some(name) = extract_let_name(trimmed)
                && name.len() > 1 {
                    identifiers.push(name);
                }
    }
    ...
    let abbr_ratio = f64::from(abbreviation_count) / identifiers.len() as f64;
    let abbr_score = (abbr_ratio * 100.0).clamp(0.0, 50.0);

    let single_char_ratio = f64::from(single_char_count) / identifiers.len() as f64;
    let single_char_score = (single_char_ratio * 100.0).clamp(0.0, 30.0);

    let inconsistent_ratio = f64::from(inconsistent_count) / identifiers.len() as f64;
    let inconsistent_score = (inconsistent_ratio * 100.0).clamp(0.0, 30.0);

    

    (length_score.mul_add(0.25, abbr_score * 0.25) + single_char_score * 0.25 + inconsistent_score * 0.25)
        .clamp(0.0, 100.0)
}
...
fn is_abbreviation(s: &str) -> bool {
    ...
    let upper_count = s.chars().filter(char::is_ascii_uppercase).count();
...
pub fn calculate_coupling(files: &[FileCouplingInfo]) -> f64 {
    ...
    (in_score * 0.2 + out_score * 0.2 + cross_layer_score * 0.35 + circular_score * 0.25)
        .clamp(0.0, 100.0)
}
...
pub fn calculate_test_entropy(files: &[TestFileInfo]) -> f64 {
    ...
    distribution_score.mul_add(0.25, coverage_score * 0.5 + assertion_score * 0.25)
        .clamp(0.0, 100.0)
}
...
pub fn calculate_overall_score(dimensions: &EntropyDimensions, weights: &DimensionWeights) -> f64 {
    dimensions.test.mul_add(weights.test, dimensions.naming.mul_add(weights.naming, dimensions.coupling.mul_add(weights.coupling, dimensions.complexity.mul_add(weights.complexity, dimensions.structural * weights.structural))))
}
...
pub fn aggregate_dimensions(file_scores: &[f64]) -> f64 {
    ...
    let max = file_scores.iter().copied().fold(f64::NAN, f64::max);
    let p90 = percentile_90(file_scores);

    p90.mul_add(0.3, max.mul_add(0.3, avg * 0.4)).clamp(0.0, 100.0)
}
...
fn percentile_90(values: &[f64]) -> f64 {
    ...
    let idx = usize::try_from(((values.len() as f64 - 1.0) * 0.9).round() as i64).unwrap_or_default();
...
";
    ...
    fn test_overall_score_weights() {
        ...
        let expected = 50.0f64.mul_add(0.15, 10.0f64.mul_add(0.15, 30.0f64.mul_add(0.20, 40.0f64.mul_add(0.25, 20.0 * 0.25))));
...
impl SystemEntropyReport {
    ...
    fn recalculate(&mut self) {
        ...
        for cell in &self.cells {
            ...
            weighted.structural = cell.dimensions.structural.mul_add(weight, weighted.structural);
            weighted.complexity = cell.dimensions.complexity.mul_add(weight, weighted.complexity);
            weighted.coupling = cell.dimensions.coupling.mul_add(weight, weighted.coupling);
            weighted.naming = cell.dimensions.naming.mul_add(weight, weighted.naming);
            weighted.test = cell.dimensions.test.mul_add(weight, weighted.test);
            weighted_overall = cell.overall_score.mul_add(weight, weighted_overall);
        }
