...
impl ExperimentStatus {
    ...
    pub fn can_transition_to(&self, target: &Self) -> bool {
        matches!(
            (self, target),
            (Self::Draft | Self::Paused, Self::Running)
                | (Self::Draft | Self::Completed, Self::Archived)
...
impl Experiment {
    ...
    pub fn variant_allocations(&self) -> Vec<VariantAllocation> {
        let total_weight = f64::from(self.total_weight());
        if total_weight == 0.0 {
            return Vec::new();
        }
        self.variants
            ...
            .map(|v| VariantAllocation {
                ...
                allocated_users: self.metrics.get(&v.id).map_or(0, |m| m.total_users),
                allocation_percentage: f64::from(v.weight) / total_weight * 100.0,
            })
            .collect()
    }
}
...
impl std::fmt::Display for ABTestError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::ExperimentAlreadyTerminal(id) => write!(f, "Experiment is already in terminal state: {id}"),
            Self::VariantNotFound(id) => write!(f, "Variant not found: {id}"),
            Self::NoControlVariant(id) => write!(f, "No control variant in experiment: {id}"),
            Self::InsufficientVariants(msg) => write!(f, "Insufficient variants: {msg}"),
            Self::InvalidTrafficPercentage(msg) => write!(f, "Invalid traffic percentage: {msg}"),
...
impl ABTestManager {
    ...
    pub fn create_experiment(
        ...
        if !(0.0..=100.0).contains(&traffic_percentage) {
            return Err(ABTestError::InvalidTrafficPercentage(format!(
                "Traffic percentage must be between 0 and 100, got {traffic_percentage}"
            )));
        }
    ...
    pub fn get_variant(&self, experiment_id: Uuid, user_id: &str) -> Result<Option<&Variant>, ABTestError> {
        ...
        let total_weight = u64::from(experiment.total_weight());
        if total_weight == 0 {
            return Ok(None);
        }

        let variant_hash = hash_user_id(user_id, &experiment.id) ^ 0x9E37_79B9_7F4A_7C15;
...
mod tests {
    ...
    fn test_different_users_different_variants() {
        ...
        assert!(control_count > 0, "Expected some control users");
        assert!(treatment_count > 0, "Expected some variant users");
        assert_eq!(control_count + treatment_count, 1000);
    }
    ...
    fn test_traffic_percentage_filtering() {
        ...
        let ratio = f64::from(in_experiment) / f64::from(total) * 100.0;
        assert!(
            (ratio - 10.0).abs() < 5.0,
            "Expected ~10% traffic, got {ratio}% ({in_experiment} out of {total})"
        );
    }
