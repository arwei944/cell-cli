use crate::dependency_graph::{DependencyGraph, EdgeType, NodeId, NodeType};
...
impl RootCauseType {
    pub fn label(&self) -> &str {
        match self {
            ...
            Self::PerformanceDegradation => "Performance Degradation",
            Self::SecurityBreach => "Security Breach",
            Self::DataInconsistency => "Data Inconsistency",
            Self::DeploymentFailure => "Deployment Failure",
    ...
    pub fn base_confidence(&self) -> f64 {
        match self {
            Self::ServiceDown => 0.9,
            Self::ResourceExhaustion | Self::DependencyFailure => 0.8,
            Self::DatabaseIssue | Self::DeploymentFailure => 0.85,
            Self::NetworkIssue | Self::CodeBug | Self::DataInconsistency => 0.7,
            Self::ConfigurationError => 0.75,
            Self::PerformanceDegradation => 0.65,
            Self::SecurityBreach => 0.6,
            Self::Unknown => 0.3,
        }
    }
}
...
impl RCAEngineV2 {
    ...
    pub fn analyze(&self, signals: &[RCASignal]) -> RCAResultV2 {
        ...
        let mut candidates: Vec<RootCauseCandidate> = component_scores
            ...
            .map(|(component, raw_score)| {
                ...
                let adjusted_confidence = multi_dim_score.overall.mul_add(0.4, confidence * 0.6).min(1.0);
    ...
    pub fn analyze_with_graph(
        ...
        for candidate in &result.candidates {
            ...
            let impact_rating = match &impact_scope {
            ...
            updated.multi_dimension_score.impact_score = impact_rating;
            ...
            let graph_bonus = (dep_strength * 0.15 + impact_rating * 0.1).min(0.2);
    ...
    fn calculate_multi_dimension_score(
        ...
        let overall = (correlation_score.mul_add(0.2, signal_strength * 0.35 + count_score * 0.15)
            + temporal_score * 0.15
    ...
    fn combine_scores(&self, score: &MultiDimensionScore) -> f64 {
        score.temporal_score.mul_add(0.15, score.impact_score.mul_add(0.15, score.dependency_strength.mul_add(0.20, score.correlation_score.mul_add(0.20, score.signal_strength * 0.30))))
            .min(1.0)
    }
    ...
    fn calculate_impact_scope(
        ...
        let risk_score = (queues.len() as f64).mul_add(8.0, (databases.len() as f64).mul_add(20.0, (services.len() as f64).mul_add(10.0, blast_radius as f64 * 15.0)))
            .min(100.0);
...
mod tests {
    use super::*;
    use crate::dependency_graph::{DependencyEdge, DependencyNode};
