...
impl EvolutionMetrics {
    ...
    pub fn calculate_overall_score(&mut self) -> f64 {
        let mut score = 100.0;
        score = f64::from(self.architecture_violations).mul_add(-5.0, score);
        score = self.entropy_score.mul_add(-2.0, score);
        score = self.handoff_completeness_score.mul_add(10.0, score);
        score = f64::from(self.total_improvements_applied).mul_add(0.5, score);
